...
export default function Settings({ currentUser: _currentUser }: SettingsProps) {
  ...
  async function handleToggleCanApprove(admin: AdminUser) {
    const newVal = !admin.can_approve_orders
    const { error } = await supabase
      .from('admin_users')
      .update({ can_approve_orders: newVal })
      .eq('id', admin.id)

    if (error) {
      setError(error.message)
    } else {
      setSuccess(`"${admin.name}" ${newVal ? 'can now approve orders' : 'can no longer approve orders'}`)
      fetchAdmins()
    }
  }

  async function handleToggleCanReject(admin: AdminUser) {
    const newVal = !admin.can_reject_orders
    const { error } = await supabase
      .from('admin_users')
      .update({ can_reject_orders: newVal })
      .eq('id', admin.id)

    if (error) {
      setError(error.message)
    } else {
      setSuccess(`"${admin.name}" ${newVal ? 'can now reject orders' : 'can no longer reject orders'}`)
      fetchAdmins()
    }
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-6 py-3 text-center font-medium text-gray-600">Can Approve Orders</th>
            ...
            <tbody>
              {admins.map((admin) => (
                <tr key={admin.id} className="border-b border-gray-100 last:border-0">
                  ...
                  <td className="px-6 py-3 text-center">
                    <button
                      onClick={() => handleToggleCanApprove(admin)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        admin.can_approve_orders ? 'bg-green-600' : 'bg-gray-300'
                      }`}
                      title={admin.can_approve_orders ? 'Click to revoke approve permission (hides the Approve button on the tablet)' : 'Click to grant approve permission'}
                    >
                      <span
                        className={`inline-block h-4 w-4 rounded-full bg-white shadow transform transition-transform ${
                          admin.can_approve_orders ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </td>
                  ...
                  <td className="px-6 py-3 text-center">
                    <button
                      onClick={() => handleToggleCanReject(admin)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        admin.can_reject_orders ? 'bg-red-500' : 'bg-gray-300'
                      }`}
                      title={admin.can_reject_orders ? 'Click to revoke reject permission' : 'Click to grant reject permission'}
                    >
                      <span
                        className={`inline-block h-4 w-4 rounded-full bg-white shadow transform transition-transform ${
                          admin.can_reject_orders ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </td>
              ...
              {admins.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-gray-500">No admin users found</td>
                </tr>
              )}
            </tbody>
          </table>
