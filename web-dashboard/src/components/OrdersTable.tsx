        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="px-4 py-3 text-right font-medium text-gray-600">Total ex VAT</th>
              <th className="px-4 py-3 text-right font-medium text-gray-600">Total inc VAT</th>
          ...
          <tbody>
            {orders.length === 0 ? (
              ...
              orders.map((order) => {
                ...
                return (
                  <tr key={order.id}>
                    {/* Row cells in a sub-table to allow item rows */}
                      ...
                      <table className="w-full">
                        <tbody>
                          ...
                          >
                            ...
                            <td className="px-4 py-3 font-mono font-medium text-navy min-w-[120px]">
                              {bulkEdit ? (
                                <input
                                  type="text"
                                  value={docketValue}
                                  onChange={(e) => setDocket(order.id, e.target.value)}
                                  className="w-24 px-2 py-1 text-sm font-mono border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gold"
                          ...
                          {bulkEdit && order.order_items && order.order_items.length > 0 && (
                            order.order_items.map((item) => {
                              ...
                              return (
                                <tr
                                  key={item.id}
                                  className={`border-b border-gray-50 ${qtyMismatch ? 'bg-red-50' : 'bg-gray-50/50'}`}
                                >
                                  <td />
                                  <td />
                                  <td colSpan={2} className="px-4 py-1.5 text-xs text-gray-700 pl-8">
                                    {item.item_name}
                                  </td>
                                  ...
                                  <td className="px-4 py-1.5 text-right text-xs text-gray-600">
